...
<@markup id="approved-tags-resources">
   <@inlineScript group="template-common">
      (function() {
          function enhanceApprovedTags() {
              var elements = document.querySelectorAll('.tag, .tag-item, .tag-link, a[href*="tag"], .document-metadata a');

              for (var i = 0; i < elements.length; i++) {
                  var element = elements[i];
                  var text = (element.textContent || element.innerText || '').trim().toLowerCase();
          ...
          function initEnhancer() {
              ...
              if (typeof MutationObserver !== 'undefined') {
                  var observer = new MutationObserver(function() {
                      setTimeout(enhanceApprovedTags, 100);
                  });
                  observer.observe(document.body, { childList: true, subtree: true });
              }

              setInterval(enhanceApprovedTags, 2000);
          }
   ...
   <style type="text/css">
      .approved-tag-enhanced,
      .tag[title="APROBADO"],
      .tag-item[title="APROBADO"],
      .tag-link[title="APROBADO"] {
      ...
      .approved-tag-enhanced:hover,
      .tag[title="APROBADO"]:hover,
      .tag-item[title="APROBADO"]:hover,
